...
const PAYMENT_AMOUNT = 10000 * Math.pow(10, SPL402_DECIMALS); // 10000 SPL402 tokens
...
export function Verify() {
  ...
  const handleRegister = useCallback(async () => {
    ...
    if (!sendTransaction) {
      setStatus({ type: 'error', message: 'Wallet does not support sending transactions' });
      return;
    }
  ...
  }, [publicKey, sendTransaction, apiEndpoint, description, contact, connection]);
              ...
              <>
                ...
                <div className="bg-[#9945FF]/5 border border-[#9945FF]/20 rounded-xl p-4">
                  <p className="text-sm text-[#9945FF]">
                    <strong>Registration Fee:</strong> {(PAYMENT_AMOUNT / Math.pow(10, SPL402_DECIMALS)).toLocaleString()} SPL402 tokens
                  </p>
                </div>

                <button
                  onClick={handleRegister}
                  disabled={loading || !publicKey || !apiEndpoint || !description || !contact}
